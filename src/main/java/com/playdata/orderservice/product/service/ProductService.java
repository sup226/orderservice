...
import static com.playdata.orderservice.product.entity.QProduct.product;
...
/*        File file = new File("C:/Users/playdata2/Desktop/develop/upload/" + uniqueFileName);
        try {
    ...
    public Page<ProductResDto> productList(ProductSearchDto searchDto, Pageable pageable) {
        ...
        // QueryDSL 로 조회한 내용을 모두 포함하는 Page 객체 생성.
        Page<Product> productPage = new PageImpl<>(products, pageable, total);

        // 엔터티를 일괄적으로 dto로 변환하기
        Page<ProductResDto> productResDtos = productPage.map(p -> p.fromEntity());
